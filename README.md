...
Supported live constructs are ATX headings, emphasis, strong emphasis, inline links, circular bullet lists, horizontal rules, interactive task lists, GFM tables, inline and syntax-highlighted fenced code, Markdown images, and Mermaid fences. Table cells edit their raw inline Markdown directly inside the rendered grid, and ordinary fenced code edits inside its rendered card. Vim `j` and `k` move by exactly one document line, including through rendered blocks. Highlighting is loaded lazily for Ruby, JavaScript/TypeScript, JSX/TSX, JSON, HTML/CSS, Python, Rust, Go, Bash, SQL, YAML, and Markdown. Line numbers and the Vim leader key can be changed from the `Ctrl+,` settings modal.
